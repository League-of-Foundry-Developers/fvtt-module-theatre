...
class Theatre {
	...
	_sendTypingEvent() {
		...
		let insertTextFlyin = insert ? this._getTextFlyinFromInsert(insert) : this.speakingAs == Theatre.NARRATOR ? this.theatreNarrator.getAttribute("textflyin") : "typewriter";
		let insertTextStanding = insert
			...
			: "none";
		let insertTextFont = insert ? this._getTextFontFromInsert(insert) : this.speakingAs == Theatre.NARRATOR ? this.theatreNarrator.getAttribute("textfont") : null;
		let insertTextSize = insert ? this._getTextSizeFromInsert(insert) : this.speakingAs == Theatre.NARRATOR ? this.theatreNarrator.getAttribute("textsize") : null;
		let insertTextColor = insert ? this._getTextColorFromInsert(insert) : this.speakingAs == Theatre.NARRATOR ? this.theatreNarrator.getAttribute("textcolor") : null;
	...
	_getInsertParamsFromActorId(actorId) {
		...
		if (actor.flags.theatre) {
			...
			if (actor.flags.theatre.baseinsert && actor.flags.theatre.baseinsert != "") portrait = actor.flags.theatre.baseinsert;
	...
	async renderInsertById(id) {
		...
		if (insert.emote && params.emotes[insert.emote].insert && params.emotes[insert.emote].insert != "") resName = params.emotes[insert.emote].insert;
	...
	_repositionInsertElements(insert) {
		...
		if (insert.nameOrientation == "left") {
			...
			if (labelExceeds) {
				...
				else insert.typingBubble.x = Math.max(insert.portrait.width - insert.label.width - insert.typingBubble.texture.width / 2 - 20, insert.typingBubble.width / 2);
			} else {
				...
				else insert.typingBubble.x = Math.max(insert.portrait.width - preLabelWidth - insert.typingBubble.texture.width / 2 - 20, insert.typingBubble.width / 2);
			}
		...
		insert.typingBubble.y =
			insert.portrait.height - (insert.optAlign == "top" ? 0 : Theatre.instance.theatreBar.offsetHeight) - insert.label.style.lineHeight + insert.typingBubble.height / 2;
	...
	_clearPortraitContainer(imgId) {
		...
		for (let tweenId in insert.tweens) {
			if (tweenId == "nameSpeakingPulse" || tweenId == "typingBounce" || tweenId == "typingAppear" || tweenId == "typingVanish" || tweenId == "typingWiggle") continue;
		...
		for (let idx = insert.dockContainer.children.length - 1; idx >= 0; --idx) {
			...
			else if (child.theatreComponentName && child.theatreComponentName == "typingBubble") insert.dockContainer.removeChildAt(idx);
			else child.destroy();
		}
		insert.portrait = null;
		insert.portraitContainer = null;
	...
	_removeInsert(toRemoveInsert, toRemoveTextBox, remote) {
		...
		for (let navItem of this.theatreNavBar.children)
			if (navItem.getAttribute("imgId") == toRemoveInsert.imgId) {
				...
				if (toRemoveInsert.imgId == this.speakingAs) KHelpers.removeClass(navItem, "theatre-control-nav-bar-item-speakingas");
			}
	...
	_getInitialEmotionSetFromInsertParams(params, useDefault) {
		...
		let emotions = {
			...
			textFlyin:
				(!useDefault && params.settings.emote && params.emotes[params.settings.emote] && params.emotes[params.settings.emote].settings
	...
	toggleNarratorBar(active, remote) {
		if (active) {
			...
			if (game.user.isGM) {
				...
				Theatre.instance.theatreNarrator.setAttribute(
					"textsize",
					textSize ? textSize : Theatre.instance.userEmotes[game.user.id] ? Theatre.instance.userEmotes[game.user.id].textSize : null
				);
				Theatre.instance.theatreNarrator.setAttribute(
					"textcolor",
	...
	renderEmoteMenu() {
		...
		renderTemplate("modules/theatre/app/templates/emote_menu.html", {
			...
			else if (Theatre.instance.userEmotes[game.user.id]) sizeValue = Theatre.instance.userEmotes[game.user.id].textSize;
	...
	handleWindowResize(ev) {
		...
		// emote menu
		if (Theatre.instance.theatreEmoteMenu) Theatre.instance.theatreEmoteMenu.style.top = `${Theatre.instance.theatreControls.offsetTop - 410}px`;
		/*
	...
	handleBtnDelayEmoteClick(ev) {
		...
		if (Theatre.instance.isDelayEmote) {
			if (KHelpers.hasClass(ev.currentTarget, "theatre-control-small-btn-down")) KHelpers.removeClass(ev.currentTarget, "theatre-control-small-btn-down");
			Theatre.instance.isDelayEmote = false;
		} else {
			if (!KHelpers.hasClass(ev.currentTarget, "theatre-control-small-btn-down")) KHelpers.addClass(ev.currentTarget, "theatre-control-small-btn-down");
	...
	static getFonts() {
		if (!Theatre.FONTS) {
			...
			for (let idx = Theatre.FONTS.length - 1; idx >= 0; --idx) {
				if (Theatre.FONTS[idx] == Theatre.instance.titleFont || Theatre.FONTS[idx] == Theatre.instance.textFont) continue;
